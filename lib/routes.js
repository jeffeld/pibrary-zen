...
module.exports = function (app, config) {
    ...
    app.param ('stockcode2', function (req, res, next, stockcode2){

        var rxid = /\b[0-9A-F]{10}\b/gi;
        if (stockcode2.match(rxid)) {

            orac.GetByStockCode2(stockcode2).then(function (data) {

                if (_.isEmpty(data)) {
                    res.send (404);
                } else {

                    req.zen.stockItem = data;
    ...
    // Validate the membershipcode parameter is valid and load the record into
    // the request object if it is.

    app.param ('membershipcode', function (req, res, next, membershipcode){

        var rxid = /^B{1}\d{6}$/i;

        if (membershipcode.match(rxid)) {

            membersapi.GetByMembershipId2 (membershipcode).then(function (data) {
    ...
    var userAppSettings = {
            brand : process.env.BRAND || "Pibrary",
            isUserApp : true,
        ...
        prepareRoutes = function (routes, middleware) {
            ...
            _.each (routes, function (r) {
                ...
                app.get(r.route, middleware, function (req, res) {
                    ...
                    if (_.isFunction(r.preRender)) {
                        r.preRender (req, res, s);
                    }
...
//    });
    ...
    var consumerPagesForGet = [
        {
            ...
            config: function (req, res) {
                return {
                    landing : {
                        heading: process.env.LANDING_HEADING || 'Welcome to The Library',
                        p1: process.env.LANDING_P1 || 'Main intro heading here',
                        p2: process.env.LANDING_P2 || 'By line here',
                        p3: process.env.LANDING_P3 || 'Small message here',
                    },
    ...
    app.put ('/api/lend/:stockcode2/:membershipcode' /*, ensureIsAdmin*/, actionsapi.LendItem);
    app.put ('/api/force/:stockcode2/:membershipcode', ensureIsAdmin, actionsapi.ForceLendItem);
    ...
    app.get ('/api/counts/overdues', ensureIsAdmin, actionsapi.GetOverdueCount);
    ...
    app.get('/api/overdues', ensureIsAdmin, actionsapi.GetOverdues);
