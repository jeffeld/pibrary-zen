    ...
    if (req.body.activationInfo.photo === 'data:,') {
        ...
        var photo = req.body.activationInfo.photo,
            ...
            promises = [
                ...
                util.UploadToS3 (req.body.activationInfo.card + '.' + ext, imageData),
...
exports.GetMemberPhoto = function (req, res) {
    ...
    const noMugshot = "/images/No Member-320x240.jpg";
    const pathToImage = `${process.env.MUGSHOTS_PATH}/${process.env.S3_MUGSHOTS_BUCKET}/${req.params.membershipcode}.png`;

    restler.head (pathToImage)
        .on("200", () => {
            res.redirect (pathToImage);
        })
        .on("fail", () => {
            res.redirect (noMugshot);
        });
